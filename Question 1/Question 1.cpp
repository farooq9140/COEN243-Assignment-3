...
int main() {
	...
	cout << "Grade Median is: " << median(grades, gradeCount) << endl;
...
double median(int* grades, int gradeCount) {
	...
	sort(grades, grades + gradeCount);
	if (gradeCount % 2 == 0) {
		medianValue = (static_cast<double>(grades[(gradeCount / 2-1)]) + grades[(gradeCount / 2)])/2;
	}
	else {
		medianValue = grades[(gradeCount / 2)];
	}
	return medianValue;
}
